...
HopWhy publishes native archives for Linux x86_64, macOS Apple Silicon and
...
archive="hopwhy-${version}-${platform}.tar.gz"
gh release download "$version" --repo yhay81/hopwhy \
  --pattern "$archive" --pattern SHA256SUMS
awk -v file="./$archive" \
  '$2 == file { print; found = 1 } END { if (!found) exit 1 }' \
  SHA256SUMS | checksum -c -
gh attestation verify "$archive" --repo yhay81/hopwhy
tar -xzf "$archive"
install -d "$HOME/.local/bin"
install -m 0755 "${archive%.tar.gz}/hopwhy" "$HOME/.local/bin/hopwhy"
"$HOME/.local/bin/hopwhy" --version
```
...
$version = "v0.3.0"
$archive = "hopwhy-$version-windows-x86_64.zip"
gh release download $version --repo yhay81/hopwhy `
  --pattern $archive --pattern "SHA256SUMS"
...
Copy-Item "hopwhy-$version-windows-x86_64\hopwhy.exe" `
  (Join-Path $bin "hopwhy.exe") -Force
& (Join-Path $bin "hopwhy.exe") --version
...
```sh
git clone https://github.com/yhay81/hopwhy.git
cd hopwhy
...
version. To remove a native installation, delete `$HOME/.local/bin/hopwhy` on
macOS/Linux or `$HOME\.local\bin\hopwhy.exe` on Windows. Saved reports and
specifications are not removed automatically.
